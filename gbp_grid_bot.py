# FINAL BOT SCRIPT - RENDER (NO FILE SAVING)
import time
...
# Portfolio is now only stored in memory. It will reset on restart.
portfolio = {
...
async def start_command(update, context):
    await update.message.reply_text("Bot is online (in-memory stats). Commands: /start, /status, /stats, /analysis", parse_mode='Markdown')
        ...
        for pair in PAIRS:
            try:
                ...
                if h1_data.empty or len(h1_data) < 2 or h4_data.empty or len(h4_data) < 1:
                    current_analysis[pair] = "Waiting for data."
                    continue
                ...
                current_analysis[pair] = f"Error: {e}"
